""" 
Provided is a list of data about a store’s inventory 
where each item in the list represents the name of an item, 
how much is in stock, and how much it costs. 
Print out each item in the list with the same formatting, 
using the .format method (not string concatenation). 
For example, the first print statment should read The store has 12 shoes, each for 29.99 USD.
"""


inventory = ["shoes, 12, 29.99", "shirts, 20, 9.99", "sweatpants, 25, 15.00", "scarves, 13, 7.75"]
for item in inventory:
    item_list = item.split(",")
    name = item_list[0]
    amount = item_list[1].strip()
    #print("type of amount: {}".format(type(amount)))
    cost = item_list[2].strip()
    #print("type of cost: {}".format(type(cost)))
    print("The store has {} {}, each for {} USD.".format(amount,name,cost))

print("#####")

inventory = ["shoes, 12, 29.99", "shirts, 20, 9.99", "sweatpants, 25, 15.00", "scarves, 13, 7.75"]

for i in inventory:
    i_lst = i.split(",")
    a = i_lst[0]
    b = i_lst[1].strip()
    c = i_lst[2].strip()
    print("The store has {} {}, each for {} USD.".format(b, a, c))

print("#####")
inventory = ["shoes, 12, 29.99", "shirts, 20, 9.99", "sweatpants, 25, 15.00", "scarves, 13, 7.75"]
for i in inventory:
    ilst = i.split(",")
    print("The store has {} {}, each for {} USD.".format(ilst[1].strip(), ilst[0].strip(), ilst[2].strip()))

